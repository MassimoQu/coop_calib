...
def inference_hybrid_fusion(batch_data, model, dataset):
    """
    Model inference for hybrid fusion.
...
def inference_hybrid_fusion_with_comm(batch_data, model, dataset):
    """
    Model inference for hybrid fusion.
    ...
    output_dict['ego'] = model(cav_content)
    comm_points = output_dict["ego"]["comm_points"]

    pred_box_tensor, pred_score, gt_box_tensor = \
        dataset.post_process(batch_data,
                             output_dict)

    return pred_box_tensor, pred_score, gt_box_tensor, comm_points
