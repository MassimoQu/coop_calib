...
class Communication(nn.Module):
    ...
    def forward(self, batch_confidence_maps, record_len, pairwise_t_matrix):
        ...
        for b in range(B): # 每一个车分别选一遍
            ...
            ones_mask = torch.ones_like(communication_maps).to(communication_maps.device)  # 生成一个全1的张量
            zeros_mask = torch.zeros_like(communication_maps).to(communication_maps.device)  # 生成一个全0张量
            ...
            ones_mask = torch.ones_like(communication_mask).to(communication_mask.device)  # torch.ones_like是生成一个与目标相同维度的全1矩阵
            communication_mask_nodiag[::2] = ones_mask[::2]  
            communication_masks.append(communication_mask_nodiag)
            communication_rates.append(communication_rate)
            batch_communication_maps.append(ori_communication_maps*communication_mask_nodiag)  # 只保留不是0的
        communication_rates = sum(communication_rates)/B
        communication_masks = torch.concat(communication_masks, dim=0) # 组合这段中每个车的
