...
class Where2comm(nn.Module):
    ...
    def regroup(self, x, record_len):
        #cumsum 是累加，所以，这里应该是按每个片段的记录来的
        #累加完了之后就是该分离的地方，用这个来分离出来每个片段的特征？
        cum_sum_len = torch.cumsum(record_len, dim=0)# 感觉应该是根据每个车相邻？车的数量来求和，查了一下，应该是 每个车检测到的车的数量
            ...
            for i in range(self.num_levels):
                ...
                if i==0:
                    if self.communication:
                        batch_confidence_maps = self.regroup(rm, record_len)# confidence maps ，六个车各一个
