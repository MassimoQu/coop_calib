...
from opencood.models.sub_modules.voxelnext import VoxelResBackBone8xVoxelNeXt3D
from opencood.models.sub_modules.spconv_sps_quantisizer import VoxelResSPSQuantiseizer
from opencood.models.fuse_modules.covot import CoVoT
...
class VoxelNeXtCoVoTHybrid(nn.Module):
    def __init__(self, args):
        super(VoxelNeXtCoVoTHybrid, self).__init__()
        ...
        # sparse 3d backbone
        self.backbone_3d = VoxelResBackBone8xVoxelNeXt3D(4, args['backbone_3d'],
                                            args['grid_size'])
        # Vote2Fuse module
        self.vote2fusion = CoVoT(args = args['fusion'])
    ...
    def backbone_fix(self):
        ...
        for param in self.backbone_3d.parameters():
            param.requires_grad = False
        for param in self.vote2fusion.parameters():
            param.requires_grad = False
        for param in self.vote2fusion.win_prob.parameters():
    ...
    def forward(self, data_dict):
        if self.raw == True:
            ...
            # data processing for cavs and ego vehicle intermediate fusion
            if len(data_dict['lidar_col_list']) > 0:
                # print('data_dict_col:',data_dict['lidar_col_list'])
                lidar_ego_list = data_dict['lidar_ego_list']
                lidar_col_list = data_dict['lidar_col_list']
                record_len = data_dict['record_len'].cpu().numpy()
                batch_size = len(record_len)
                lidar_list = []
                for i in range(batch_size):
                    lidar_list.append(lidar_ego_list[i])
                    if record_len[i] == 0:
                        continue
                    for j in range(record_len[i]):
                        lidar_list.append(lidar_col_list[0])
                        lidar_col_list.pop(0)
                batch_dict = self.extract_pts_feats(lidar_list)
                ...
                for i in range(batch_size):
                    ...
                    if num_cav_col == 0:
                        continue
                    for _ in range(num_cav_col.item()):
                        aligned_gt_boxes.append(gt_boxes_col)
                aligned_gt_boxes = torch.stack(aligned_gt_boxes, dim=0)
                # print(aligned_gt_boxes.shape)
                batch_dict.update({'record_len': data_dict['record_len'] + torch.Tensor([1]).to(data_dict['record_len'].device),
                                'gt_boxes': aligned_gt_boxes.float()})
                # print("gt_boxes:",batch_dict_col['gt_boxes'])
            else:
                lidar_list = data_dict['lidar_ego_list']
            ...
            # assert batch_dict['batch_size'] == self.batch_size
            batch_dict.update({'batch_size': len(lidar_list),})
        ...
        batch_dict = self.vote2fusion(batch_dict)
        vote_loss = batch_dict['vote_loss']
        if not self.training:
            comm_points = batch_dict["comm_points"]
        ...
        if not self.training:
            comm_points = batch_dict["comm_points"]
            output_dict.update({'comm_points': comm_points})
