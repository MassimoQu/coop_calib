...
from opencood.models.sub_modules.feature_flow import FlowGenerator    
...
class VoxelNeXtSecondHybridMulti(nn.Module):
    def __init__(self, args):
        super(VoxelNeXtSecondHybridMulti, self).__init__()
        ...
        self.delay = 1
        # feature flow
        self.STCnet = FlowGenerator(args)
    ...
    def backbone_fix(self):
        ...
        for param in self.pruning.parameters():
    ...
    def cav_pruning_fusion(self, batch_dict_col, data_dict):
        if self.pruning is not None:
            ...
            batch_dict_col = self.extract_pts_feats(compressed_point_col)
        ...
        return batch_dict, batch_dict_col
    ...
    def forward(self, data_dict_list):
        ...
        for origin_data in data_dict_list:
            ...
            if self.raw == True:
                ...
                if len(data_dict['lidar_col_list']) > 0:
                    ...
                    batch_dict, batch_dict_col = self.cav_pruning_fusion(batch_dict_col,data_dict)
                else:
                    batch_dict = self.extract_pts_feats(data_dict['lidar_ego_list'])
                    batch_dict_col = batch_dict
                assert batch_dict['batch_size'] == self.batch_size
                batch_dict.update({'batch_size': self.batch_size,
                                'gt_boxes': data_dict['object_bbx_center'].float()})
            ...
            batch_dict_col = self.mean_vfe(batch_dict_col) 
            # print num voxels
            # print('voxel_features:',batch_dict['voxel_features'].shape)      
        ...
        feature_final, stc_loss = self.STCnet(feature_2d_list)
        ...
        output_dict = {'psm': psm,
                       'rm': rm,
                       'loss_box_of_pts_sprs': batch_dict_list[0]['loss_box_of_pts_sprs'],}
                    #    'loss_box_of_pts_sprs_pruning': batch_dict['loss_box_of_pts_sprs_pruning']}
        if 'loss_box_of_pts_sprs_pruning' in batch_dict_list[0]:
            output_dict.update({'loss_box_of_pts_sprs_pruning': batch_dict_list[0]['loss_box_of_pts_sprs_pruning']})
        return output_dict