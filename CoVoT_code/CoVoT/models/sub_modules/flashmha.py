...
class FlashAttention(nn.Module):
    ...
    def forward(self, q, kv, 
        ...
        if key_padding_mask is None:
            ...
            lengths_k = (key_padding_mask == 0).sum(dim=1).to(torch.int32)
            cu_seqlens_k = torch.zeros(batch_size + 1, dtype=torch.int32, device=q.device)
            torch.cumsum(lengths_k, dim=0, out=cu_seqlens_k[1:])
            print(cu_seqlens_k.shape)
            print(batch_size)
...
class FlashMHA(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, bias=True, batch_first=True, attention_dropout=0.0,
        ...
        self.out_proj = nn.Linear(embed_dim, embed_dim, bias=bias).half()
    ...
    def forward(self, q, k, v, key_padding_mask=None):
        ...
        print(context.shape)
        return self.out_proj(rearrange(context, 'b s h d -> b s (h d)').contiguous()), attn_weights
