...
from opencood.models.sub_modules.sst_ops import flat2window_v2, window2flat_v2, get_inner_win_inds, make_continuous_inds, get_flat2win_inds_v2, get_window_coors
...
drop_info_training ={
    0:{'max_tokens':20, 'drop_range':(0, 20)},
    1:{'max_tokens':40, 'drop_range':(20, 40)},
    2:{'max_tokens':64, 'drop_range':(40, 100000)},
}
drop_info_test ={
    0:{'max_tokens':20, 'drop_range':(0, 20)},
    1:{'max_tokens':40, 'drop_range':(20, 40)},
...
class SSTInputLayerV2(nn.Module):
    ...
    def __init__(self,
        ...
        self.sparse_shape = args["sparse_shape"]
        self.shuffle_voxels = args["shuffle_voxels"]
        self.debug = args["debug"]
        self.window_shape = args["window_shape"]
        self.normalize_pos = args["normalize_pos"]
        self.pos_temperature = args["pos_temperature"]
        self.mute = mute


    # @auto_fp16(apply_to=('voxel_feat', ))
    def forward(self, batch_dict, batch_size=None):
        '''
        Args:
            voxel_feats: shape=[N, C], N is the voxel num in the batch.
            coors: shape=[N, 4], [b, z, y, x]
        ...
        '''
        self.set_drop_info()
        voxel_feats = batch_dict["encoded_spconv_tensor"].features
        voxel_coors = batch_dict["encoded_spconv_tensor"].indices
        # create res connection
        ...
        for ego_batch_id in range(record_len.shape[0]):
                self_feature_list.append(voxel_feats[voxel_coors[:,0]==cum_sum_len[ego_batch_id]])
                self_indices_list.append(voxel_coors[voxel_coors[:,0]==cum_sum_len[ego_batch_id]])
                self_indices_list[-1][:,0] = ego_batch_id
        self_feature = torch.cat(self_feature_list,dim = 0)
        self_indices = torch.cat(self_indices_list,dim = 0)
        ego_spconv_tensor = spconv.SparseConvTensor(
            features=self_feature,
            indices=self_indices,
            spatial_shape=batch_dict["encoded_spconv_tensor"].spatial_shape,
        ...
        voxel_info['voxel_feats'] = voxel_feats
        voxel_info['voxel_coors'] = voxel_coors
        voxel_info["ego_spconv_tensor"] = ego_spconv_tensor
