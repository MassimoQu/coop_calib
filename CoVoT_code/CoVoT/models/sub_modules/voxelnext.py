...
class PostActBlock(spconv.SparseModule):
    def __init__(self, in_channels, out_channels, kernel_size, indice_key=None, stride=1, padding=0, pruning_ratio=0.5, point_cloud_range=[-3, -46.08, 0, 1, 46.08, 92.16], #[0, -46.08, -3, 92.16, 46.08, 1],
                   conv_type='subm', norm_fn=None, loss_mode=None, algo=ConvAlgo.Native, pred_mode="learnable",downsample_pruning_mode="topk"):
        super().__init__()
        ...
        if conv_type == 'subm':
            ...
            self.conv = DynamicFocalPruningDownsample(in_channels, out_channels, kernel_size, stride=stride, padding=padding, indice_key=indice_key, bias=False, 
                pruning_ratio=pruning_ratio, pred_mode=pred_mode, pred_kernel_size=3, loss_mode=loss_mode, algo=algo, pruning_mode=downsample_pruning_mode, point_cloud_range=point_cloud_range)
        else:
...
class VoxelResBackBone8xVoxelNeXt(nn.Module):
    ...
    def __init__(self, input_channels, model_cfg, grid_size, spconv_kernel_sizes=[3, 3, 3, 3], \
    ...
    #     super().__init__()
        ...
        self.point_cloud_range = model_cfg.get('point_cloud_range', point_cloud_range)
...
class VoxelResBackBone8xVoxelNeXtPruning(nn.Module):
    downsample_type = ["dynamicdownsample_attn", "dynamicdownsample_attn", "dynamicdownsample_attn", "spconv", "spconv"]
    downsample_pruning_ratio = [0.75, 0.2, 0.2, 0., 0.]
        # def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
    #     super().__init__()
    #     self.model_cfg = model_cfg
    #     norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
    ...
    #     super().__init__()
        ...
        self.conv0 = SparseSequentialBatchdict(
            SparseBasicBlock(channels[0], channels[0], norm_fn=norm_fn, indice_key='res0'),
            SparseBasicBlock(channels[0], channels[0], norm_fn=norm_fn, indice_key='res0'),
            SparseBasicBlock(channels[0], channels[0], norm_fn=norm_fn, indice_key='res0'),
        )

        self.conv1 = SparseSequentialBatchdict(
            PostActBlock(channels[0], channels[1], spconv_kernel_sizes[0], norm_fn=norm_fn, padding=int(spconv_kernel_sizes[0]//2), indice_key='spconv1'),
            SparseBasicBlock(channels[1], channels[1], norm_fn=norm_fn, indice_key='res1'),
            SparseBasicBlock(channels[1], channels[1], norm_fn=norm_fn, indice_key='res1'),
        )

        self.conv2 = SparseSequentialBatchdict(
            # [1600, 1408, 41] <- [800, 704, 21]
            block(channels[1], channels[1], spconv_kernel_sizes[1], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[0]//2), pred_mode="pruning", \
                indice_key='spconv2', conv_type=self.downsample_type[0], pruning_ratio=self.downsample_pruning_ratio[0],loss_mode="focal_sprs",point_cloud_range=self.point_cloud_range),
            SparseBasicBlock(channels[1], channels[1], norm_fn=norm_fn, indice_key='res2'),
            SparseBasicBlock(channels[1], channels[1], norm_fn=norm_fn, indice_key='res2'),
        )
        ...
        self.conv3 = SparseSequentialBatchdict(
            # [800, 704, 21] <- [400, 352, 11]
            block(channels[1], channels[2], spconv_kernel_sizes[2], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[1]//2), \
                indice_key='spconv3', conv_type=self.downsample_type[1], pruning_ratio=self.downsample_pruning_ratio[1],loss_mode="focal_sprs",point_cloud_range=self.point_cloud_range),
            SparseBasicBlock(channels[2], channels[2], norm_fn=norm_fn, indice_key='res3'),
            SparseBasicBlock(channels[2], channels[2], norm_fn=norm_fn, indice_key='res3'),
        )
        ...
        self.conv4 = SparseSequentialBatchdict(
            # [400, 352, 11] <- [200, 176, 6]
            block(channels[2], channels[3], spconv_kernel_sizes[3], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[2]//2), \
                indice_key='spconv4', conv_type=self.downsample_type[2], pruning_ratio=self.downsample_pruning_ratio[2],loss_mode="focal_sprs",point_cloud_range=self.point_cloud_range),
            SparseBasicBlock(channels[3], channels[3], norm_fn=norm_fn, indice_key='res4'),
            SparseBasicBlock(channels[3], channels[3], norm_fn=norm_fn, indice_key='res4'),
        )
        ...
        self.conv5 = SparseSequentialBatchdict(
            # [200, 176, 6] <- [100, 88, 3]
            block(channels[3], channels[4], spconv_kernel_sizes[4], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[3]//2), \
                indice_key='spconv5', conv_type=self.downsample_type[3], pruning_ratio=self.downsample_pruning_ratio[3],point_cloud_range=self.point_cloud_range),
            SparseBasicBlock(channels[4], channels[4], norm_fn=norm_fn, indice_key='res5'),
            SparseBasicBlock(channels[4], channels[4], norm_fn=norm_fn, indice_key='res5'),
        )
        ...
        self.conv6 = SparseSequentialBatchdict(
            # [200, 176, 6] <- [100, 88, 3]
            block(channels[4], channels[4], spconv_kernel_sizes[4], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[3]//2), \
                indice_key='spconv6', conv_type=self.downsample_type[4], pruning_ratio=self.downsample_pruning_ratio[4],point_cloud_range=self.point_cloud_range),
            SparseBasicBlock(channels[4], channels[4], norm_fn=norm_fn, indice_key='res6'),
            SparseBasicBlock(channels[4], channels[4], norm_fn=norm_fn, indice_key='res6'),
        )
    ...
    def forward(self, batch_dict):
        ...
        x = self.conv_input(input_sp_tensor) # 41X1440X1440x16 33954 
        x_conv0, batch_dict = self.conv0(x, batch_dict)
        x_conv1, batch_dict = self.conv1(x, batch_dict) # 41X1440X1440x16 33954 
        vis_importance = False
        if vis_importance:
            pruning_ratio = 0.8
...
class VoxelResBackBone8xVoxelNeXtSparseFusion(nn.Module):
    downsample_type = ["dynamicdownsample_attn", "dynamicdownsample_attn", "dynamicdownsample_attn", "spconv", "spconv"]
    downsample_pruning_ratio = [0.5, 0.5, 0.5, 0., 0.]
        # def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
    #     super().__init__()
    ...
    def forward(self, batch_dict, batch_dict_col):
        """
        Args:
            batch_dict:
                batch_size: int
...
class VoxelResBackBone8xVoxelNeXt3D(nn.Module):
    downsample_type = ["dynamicdownsample_attn", "dynamicdownsample_attn", "dynamicdownsample_attn", "spconv", "spconv"]
    downsample_pruning_ratio = [0.5, 0.5, 0.5, 0., 0.]
        # def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
    #     super().__init__()
    ...
    #     super().__init__()
        ...
        self.conv_out = spconv.SparseSequential(
            # [200, 150, 5] -> [200, 150, 2]
            spconv.SparseConv2d(channels[4], out_channel, 3, stride=1, padding=1, bias=False, indice_key='spconv_down2', algo=ConvAlgo.Native),
            norm_fn(out_channel),
            nn.ReLU(),
        )
