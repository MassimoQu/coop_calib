...
from .voxelnext import SparseBasicBlock2D
...
class AST(nn.Module):
    ...
    def __init__(
        ...
        spconv.SparseConv2d(d_model[-1], vote_channel[0], 3, stride=2, padding=1, bias=False, indice_key='voter_encode0'),
        nn.BatchNorm1d(vote_channel[0]),
        nn.ReLU(True),
        SparseBasicBlock2D(vote_channel[0],vote_channel[0],norm_fn=norm_fn, indice_key='res_enc0'),
        spconv.SparseConv2d(vote_channel[0], vote_channel[1], 3, stride=2, padding=1, bias=False,indice_key='voter_encode1'),
        nn.BatchNorm1d(vote_channel[1]),
        nn.ReLU(True),
        SparseBasicBlock2D(vote_channel[1],vote_channel[1],norm_fn=norm_fn, indice_key='res_enc1'),
        spconv.SparseConv2d(vote_channel[1], vote_channel[2], 3, stride=2, padding=1, bias=False,indice_key='voter_encode2'),
        nn.BatchNorm1d(vote_channel[2]),
        nn.ReLU(True),
        SparseBasicBlock2D(vote_channel[2],vote_channel[2],norm_fn=norm_fn, indice_key='res_enc2'),
        )

        self.win_prob = nn.Sequential(nn.Linear(vote_channel[-1], 128),nn.ReLU(True),nn.Linear(128, 1),nn.BatchNorm1d(1),nn.ReLU(True))
        
        self.sparse_voter_decoder_with_vic = spconv.SparseSequential(
        spconv.SparseConvTranspose2d(vote_channel[2], vote_channel[1], 3 ,stride = 2, padding = 1, bias=False, indice_key='voter_decode0'),
        nn.BatchNorm1d(vote_channel[1]),
        nn.ReLU(True),
        SparseBasicBlock2D(vote_channel[1],vote_channel[1],norm_fn=norm_fn, indice_key='res_dec0'),
        spconv.SparseConvTranspose2d(vote_channel[1], vote_channel[0], 3 , stride = 2, padding = 1, bias=False,indice_key='voter_decode1'),
        nn.BatchNorm1d(vote_channel[0]),
        nn.ReLU(True),
        SparseBasicBlock2D(vote_channel[0],vote_channel[0],norm_fn=norm_fn, indice_key='res_dec1'),
        spconv.SparseConvTranspose2d(vote_channel[0], d_model[-1], 3 , stride = 2, padding = 1, bias=False, indice_key='voter_decode2'),
    ...
    def forward(self, batch_dict):
        ...
        if self.to_bev:
            output = self.recover_bev(output, voxel_info_col['voxel_coors'], batch_size_fused)
    ...
    def voting_regroup(self, x_input, record_len):
        ...
        x_kept, vote_loss = self.voting_info_confirm(x_voter,record_len)
