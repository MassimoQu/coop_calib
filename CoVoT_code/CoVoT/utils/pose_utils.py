...
def add_noise_data_dict(data_dict, noise_setting, seed=None):
...
def generate_noise(pos_std, rot_std, pos_mean=0, rot_mean=0, seed=None):
    ...
    if seed is not None:
        np.random.seed(seed)
    xy = np.random.normal(pos_mean, pos_std, size=(3))
    yaw = np.random.normal(rot_mean, rot_std, size=(3))

    pose_noise = np.array([xy[0], xy[1], xy[2], 0, yaw[1], 0])
    return pose_noise
