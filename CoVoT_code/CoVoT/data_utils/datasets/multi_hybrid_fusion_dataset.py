...
class HybridFusionDataset(basedataset.BaseDataset):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True):
        super(HybridFusionDataset, self). \
    ...
    def __getitem__(self, idx):
        ...
        for index, base_data_dict in select_dict.items():
            ...
            # processed_features_ego = []
            # processed_features_col = []
            lidar_ego = []
            lidar_col = []
            object_stack = []
            object_id_stack = []
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                # if cav_id == ego_id:
                #     processed_features_ego.append(
                #         selected_cav_processed['processed_features'])
                # else:
                #     processed_features_col.append(
                #         selected_cav_processed['processed_features'])
                if cav_id == ego_id:
            ...
            # cav_num = len(processed_features_col)
            # ego_feature_dict = self.merge_features_to_dict(processed_features_ego)
            # merged_feature_dict = self.merge_features_to_dict(processed_features_col)

            # augment the data for hybrid fusion
            ...
            lidar_col = [torch.from_numpy(lidar_col[i]) for i in range(cav_num)]
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        # lidar_tensor = torch.from_numpy(lidar_np).float()
    ...
    def collate_batch_train(self, batch):
        ...
        object_bbx_mask = []
        object_ids = []
        # processed_lidar_ego_list = []
        # processed_lidar_col_list = []
        lidar_ego_list = []
        lidar_col_list = []
        ...
        for i in range(len(batch)):
            ...
            # processed_lidar_ego_list.append(ego_dict['ego_lidar'])
            # processed_lidar_col_list.append(ego_dict['processed_lidar'])
        ...
        # np.array([3,5,6]), ...]}
        # ego_feature_dict = self.merge_features_to_dict(processed_lidar_ego_list)
        # merged_feature_dict = self.merge_features_to_dict(processed_lidar_col_list)
        # ego_lidar_torch_dict = \
        #     self.pre_processor.collate_batch(ego_feature_dict)
        # processed_lidar_torch_dict = self.pre_processor.collate_batch(merged_feature_dict)\
        #     if len(merged_feature_dict) > 0 else None
        # [2, 3, 4, ..., M], M <= max_cav
        ...
        output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                ...
                                #    'ego_lidar': ego_lidar_torch_dict,
