...
class HybridFusionDatasetDAIR(Dataset):
    """
    This class is for intermediate fusion where each vehicle transmit the
    deep features to ego.
    """
    def __init__(self, params, visualize, train=True, test=False):
        self.params = params
        self.visualize = visualize
        self.train = train
        self.data_augmentor = DataAugmentor(params['data_augment'],
                                            train)
        self.traincalss=params['trainclass']
        print(self.traincalss)
        ...
        split_file = load_json(params['split_dir'])
        
        if self.train:
            split_dir = split_file['vehicle_split']['train']
        else:
            split_dir = split_file['vehicle_split']['val']


        # if self.traincalss!='Car' and self.train:
        #     split_dir = 
        ...
        for frame_info in co_datainfo:
            ...
            if veh_frame_id in split_dir:
                self.split_info.append(veh_frame_id)
        self.split_info.sort()
    # co_data是最基本的车和路信息
    # 字典里包含四个路径，还有标注的路径，注意id都是以车端为准的
    #  用于无标签测试，默认不是测试，不影响以前的代码
        self.test=test
    ...
    def retrieve_base_data(self, idx):
        ...
        selected_vehicles=[]
        for object in data[0]['params']['vehicles']:
            if  self.traincalss=='all'or object['type']==self.traincalss:
                selected_vehicles.append(object)
                #data[0]['params']['vehicles'].remove(object)
        data[0]['params']['vehicles']=selected_vehicles
        ...
        transformation_matrix = veh_side_rot_and_trans_to_trasnformation_matrix(lidar_to_novatel_json_file,novatel_to_world_json_file) # 返回4x4的平移旋转矩阵，左上角3x3是旋转，右边是平移，右下角是1
        # 车端雷达到世界坐标系是这个变换矩阵
        data[0]['params']['lidar_pose'] = tfm_to_pose(transformation_matrix) # 从坐标变换矩阵得到，从平移旋转矩阵得到坐标系2相对于1的位姿，就是x,y,z,roll,yaw,pitch,1x6向量
        ...
        if self.test==False:
            ...
            selected_vehicles_veh=[]
            for object in data[0]['params']['vehicles_single']:
                if self.traincalss=='all'or object['type']==self.traincalss:
                    selected_vehicles_veh.append(object)
                    #data[0]['params']['vehicles'].remove(object)
            data[0]['params']['vehicles_single']=selected_vehicles_veh
        ######################## Single View GT ########################

        data[0]['lidar_np'] = pcd_utils.pcd_to_np(os.path.join(self.root_dir,frame_info["vehicle_pointcloud_path"]))
        if self.clip_pc:
        ...
        data[1]['params']['lidar_pose'] = tfm_to_pose(transformation_matrix1)# 应该是从变换矩阵到姿态角
        # 这里是路端雷达到世界坐标系的变换
        ######################## Single View GT ########################
        if self.test==False:
            ...
            selected_vehicles_inf=[]
            for object in data[1]['params']['vehicles_single']:
                if  self.traincalss=='all'or  object['type']==self.traincalss:
                    selected_vehicles_inf.append(object)
                    #data[0]['params']['vehicles'].remove(object)
            data[1]['params']['vehicles_single']=selected_vehicles_inf
        ######################## Single View GT ########################

        data[1]['lidar_np'] = pcd_utils.pcd_to_np(os.path.join(self.root_dir,frame_info["infrastructure_pointcloud_path"]))

        # 新加的，就是样本对应的文件名
        data[0]['frame_id'] = veh_frame_id
        data[1]['frame_id'] = inf_frame_id
        return data
    ...
    def generate_object_center_single(self,
                               cav_contents,
                               reference_lidar_pose,trainclass, return_visible_mask=False):
        return self.post_processor.generate_object_center_dairv2x_late_fusion(cav_contents,trainclass)
    
    def get_item_single_car(self, selected_cav_base, ego_pose, ego_pose_clean, cav_id, ego_keypoints, ego_allpoints, idx):
        """
        ...
        transformation_matrix_clean = \
            x1_to_x2(selected_cav_base['params']['lidar_pose_clean'],
                     ego_pose_clean)
        #比较奇怪的是这里还要变回平移旋转矩阵，因为姿态已经有了噪声，所以后边的矩阵是有噪声的
        ...
        object_bbx_center_single, object_bbx_mask_single, object_ids_single = self.generate_object_center_single([selected_cav_base],
                                                    ego_pose_clean,trainclass=self.traincalss)
    ...
    def __getitem__(self, idx):
        # 用途是把对象看做一个数组
        base_data_dict = self.retrieve_base_data(idx)  # 应该先看这个函数，基础的信息

        base_data_dict = add_noise_data_dict(base_data_dict,self.params['noise_setting'])

        # 往lidar_pose这项加定位噪声，原始的平移旋转矩阵没有被返回在data_dict 里，后边的平移旋转矩阵是通过这个姿态变换回去的，所以有噪声
        ...
        for cav_id, cav_content in base_data_dict.items():
            if cav_content['ego']:# ego表示车端，否则是路端
                ego_id = cav_id
        ...
        # 只看固定距离决定能否通信
        try:
            alignment = load_json(self.root_dir+'cooperative/newoffsets/'+str(ego_id)+'.json')
        except:
            alignment=None
        # 这里有很强的特殊性，id是0就是本车，1是路测设备
        ...
        for cav_id in cav_id_list:
            ...
            selected_cav_processed = self.get_item_single_car(
                ...
                ego_allpoints,
                idx)
            #  这一步是把坐标的标签都变换到本车，从而模拟出本车的数据，对于dairv2x就是把路侧变换到本车
            object_stack.append(selected_cav_processed['object_bbx_center']) # 添加其他车角度的物体
            ...
            if cav_id == 0:
                ...
                lidar_ego.append(selected_cav_processed['projected_lidar'])
            else:
                ...
                lidar_col.append(selected_cav_processed['projected_lidar'])
            ######################## Single View GT ########################
        ...
        object_bbx_center, lidar_ego, lidar_col, mask = \
            self.augment_hybrid(lidar_ego, lidar_col, object_bbx_center, mask)
        ...
        cav_num = len(lidar_col)
        # convert np lidar to torch tensor
        lidar_ego = [torch.from_numpy(lidar_ego[i]) for i in range(len(lidar_ego))]
        # print(len(lidar_ego), len(lidar_col))
        lidar_col = [torch.from_numpy(lidar_col[i]) for i in range(len(lidar_col))]
        ...
        processed_data_dict['ego'].update(
            ...
            #  'processed_lidar': merged_feature_dict,
             ...
             'veh_frame_id':base_data_dict[0]['frame_id'],
             'inf_frame_id':base_data_dict[1]['frame_id'],
             
             'lidar_ego_list': lidar_ego,
             'lidar_col_list': lidar_col,
    ...
    def augment_hybrid(self, lidar_ego, lidar_col, object_bbx_center, mask):
        temp_dict = {'object_bbx_center': object_bbx_center,
                        'object_bbx_mask': mask,
                        'lidar_np': lidar_ego[0],
                        'lidar_col_list': lidar_col}
        temp_dict = self.data_augmentor.forward(temp_dict)
        
        lidar_ego[0] = temp_dict['lidar_np']
        lidar_col = temp_dict['lidar_col_list']
        object_bbx_center = temp_dict['object_bbx_center']
        mask = temp_dict['object_bbx_mask']
        return object_bbx_center, lidar_ego, lidar_col, mask
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            for lidar in ego_dict['lidar_ego_list']:
                lidar_ego_list.append(lidar)
            for lidar in ego_dict['lidar_col_list']:
                lidar_col_list.append(lidar)
        ...
        ######################## Frame ID ######################## 
            veh_frame_id.append(ego_dict['veh_frame_id'])
            inf_frame_id.append(ego_dict['inf_frame_id'])
        ...
        output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                   ...
                                   'lidar_ego_list': lidar_ego_list,
                                   'lidar_col_list': lidar_col_list,},
                                   )
    ...
    def get_pairwise_transformation(self, base_data_dict, max_cav,alignment=None):
        """
        ...
        if self.proj_first:
            ...
            for cav_id, cav_content in base_data_dict.items():
                ...
                t_list.append(x_to_world(lidar_pose))  # Twx，先得到i和j到世界坐标系的矩阵
        ...
        if alignment!=None and alignment[0]<2 and alignment[0]>-2:
            pairwise_t_matrix[i, j] [1,4]=alignment[0]
            pairwise_t_matrix[i, j] [2,4]=alignment[1]
        return pairwise_t_matrix  # 是一个两两的变换矩阵，和感知到的车的总数有关系
    ...
    def post_process(self, data_dict, output_dict,vehicle_view=False):
        """
        ...
        pred_box_tensor, pred_score = \
            self.post_processor.post_process(data_dict, output_dict,vehicle_view)
        gt_box_tensor = self.post_processor.generate_gt_bbx(data_dict,vehicle_view)
