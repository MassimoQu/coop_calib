...
class PointPillarLoss(nn.Module):
    ...
    def forward(self, output_dict, target_dict, train = True):
        """
        Parameters
        ----------
        output_dict : dict
        ...
        if train == False:
            return total_loss
        if 'vote_loss' in output_dict:
            total_loss += output_dict['vote_loss'].item()
        if 'loss_box_of_pts_sprs' in output_dict:
            total_loss += output_dict['loss_box_of_pts_sprs']
        if 'loss_box_of_pts_sprs_pruning' in output_dict:
            total_loss += output_dict['loss_box_of_pts_sprs_pruning']

        self.loss_dict.update({'total_loss': total_loss,
                               'reg_loss': reg_loss,
                               'conf_loss': conf_loss})
                            #     'focal_loss': output_dict['loss_box_of_pts_sprs'],
                            #   'focal_loss_pruning': output_dict['loss_box_of_pts_sprs_pruning']})
        if 'vote_loss' in output_dict:
            total_loss += output_dict['vote_loss'].item()
        if 'loss_box_of_pts_sprs' in output_dict:
            self.loss_dict.update({'focal_loss': output_dict['loss_box_of_pts_sprs']})
        if 'loss_box_of_pts_sprs_pruning' in output_dict:
            self.loss_dict.update({'focal_loss_pruning': output_dict['loss_box_of_pts_sprs_pruning']})
        return total_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        writer.add_scalar('Confidence_loss', conf_loss.item(),
                          epoch*batch_len + batch_id)
        if 'focal_loss' in self.loss_dict:
            if isinstance(self.loss_dict['focal_loss'], torch.Tensor):
                writer.add_scalar('Focal_loss', self.loss_dict['focal_loss'].item(),
                              epoch*batch_len + batch_id)
        if 'focal_loss_pruning' in self.loss_dict:
            if isinstance(self.loss_dict['focal_loss_pruning'], torch.Tensor):
                writer.add_scalar('Focal_loss_pruning', self.loss_dict['focal_loss_pruning'].item(),
                              epoch*batch_len + batch_id)